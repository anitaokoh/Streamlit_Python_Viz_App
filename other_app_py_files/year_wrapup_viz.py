...
def create_fig(df,emotions_list,emotion_scale, min_content, max_content):
    """
    Creates a Plotly cutomized line chart with annotations
    Args: The user input, emotion scale measures, and the test received for annotation
    ...
    annotations = [{'x': min_content[0], 'y': min_content[1],'text':f"{min_content[2]}", 'font':dict(
                color="#e75480"), 'arrowhead':3,'arrowsize':2},
                {'x': max_content[0], 'y':max_content[1],'text':f"{max_content[2]}", 'font':dict(
                color="#e75480"), 'arrowhead':3,'arrowsize':2}
                ]
    ...
    fig.update_yaxes(range=[0, 8],
        ...
        tickvals= emotion_scale,showgrid=True
    )
    return fig

if __name__ == "__main__":
    create_fig(df, emotions_list,emotion_scale, min_content, max_content)
